...
from utilities.login_utilities import create_access_token, create_refresh_token, verify_token, authenticate_user,save_refresh_token,is_token_revoked,revoke_refresh_token,pwd_context,OAuth2PasswordRequestForm,OAuth2_scheme,login_db
...
class LoginController:
    ...
    async def login_for_token(user :OAuth2PasswordRequestForm= Depends() ) -> dict:
        ...
        await save_refresh_token(user.username,refresh_token,access_token)
...
class RefreshTokenController:
    ...
    @staticmethod
    async def refresh_access_token(access_token: str = Depends(OAuth2_scheme)) -> dict:
        """
        ...
        """
        refresh_tkn = await login_db.find_one({"access_token":access_token})
...
class LogoutController:
    ...
    """
        token = await login_db.find_one({"access_token":access_token})
