...
async def authenticate_user(username : str , password : str):
    ...
    """
    user = await students_db.find_one({"student_id":username})
    if not user :
        raise HTTPException(
            status_code = 404,
            detail = "User not found."
        )
    user_data = Student(**user).model_dump()
    if user and verify_password(password,user_data["password"]) :
        return user_data
    return None

async def save_refresh_token(username : str , refresh_token :str,access_token):
...
async def is_token_revoked(refresh_token:str):
    ...
    revoked_token = await login_db.find_one({"refresh_token":refresh_token})
...
async def revoke_refresh_token(refresh_token):
    ...
    """
    revoke_token = await login_db.find_one({"refresh_token":refresh_token})
